...
public class CertificateService {
    ...
    private byte[] processImage(byte[] imageBytes) throws IOException {
        ...
        try {
            ...
            if (bufferedImage == null) {
                System.err.println("[DEBUG] processImage: ImageIO could not decode the image format. Attempting alternative processing...");
                
                // Try forcing a common format by using iText directly
                return imageBytes;
            }
            
            System.out.println("[DEBUG] processImage: Successfully read image: " + bufferedImage.getWidth() + "x" + bufferedImage.getHeight() + ", type: " + bufferedImage.getType());
            ...
            if (!success) {
                System.err.println("[DEBUG] processImage: No appropriate image writer found for PNG format, using original bytes");
                return imageBytes;
            }
            
            byte[] result = processedImage.toByteArray();
            System.out.println("[DEBUG] processImage: Successfully converted to PNG, size: " + result.length + " bytes");
            ...
            // If processing fails, return the original image bytes instead of throwing exception
            return imageBytes;
        }
    }
    ...
    public byte[] generateCertificate(Map<String, String> attendee, String eventId) throws IOException {
      ...
      //  PageSize pageSize = PageSize.A4.rotate();
        ...
        try {
                ...
                if (template != null && template.getFontFamily() != null) {
                    ...
                    switch (fontFamily) {
                        ...
                        default:
                            ...
                            System.out.println("Unsupported font family: " + fontFamily + ". Using Times Roman instead.");
                            titleFont = PdfFontFactory.createFont(StandardFonts.TIMES_BOLD);
                            normalFont = PdfFontFactory.createFont(StandardFonts.TIMES_ROMAN);
                            italicFont = PdfFontFactory.createFont(StandardFonts.TIMES_ITALIC);
                    }
            ...
            // Title
            String titleText = template != null && template.getTitle() != null ? 
                             template.getTitle() : "CERTIFICATE";
            Color titleColor = template != null && template.getHeaderColor() != null ? 
                              parseColor(template.getHeaderColor()) : ColorConstants.BLACK;
            ...
            // Subtitle
            String subtitleText = template != null && template.getSubtitle() != null ? 
                                template.getSubtitle() : "OF ACHIEVEMENT";
            ...
            // Recipient intro
            Color textColor = template != null && template.getTextColor() != null ? 
                             parseColor(template.getTextColor()) : ColorConstants.BLACK;
            String recipientText = template != null && template.getRecipientText() != null ?
                                 template.getRecipientText() : "THIS CERTIFICATE IS PROUDLY PRESENTED TO";
            ...
            String description = template != null && template.getDescription() != null ?
                               template.getDescription() : "For actively participating in";
    ...
    private void handleBackgroundImage(Document document, PdfDocument pdf, Certificate template) {
        try {
            ...
            try {
                ...
                System.out.println("[DEBUG] handleBackgroundImage: Decoded base64 data, size: " + imageBytes.length + " bytes");
    ...
    private boolean tryBufferedImageApproach(Document document, PdfDocument pdf, Certificate template, byte[] imageBytes) {
        try {
            ...
            if (originalImage != null) {
                ...
                System.out.println("[DEBUG] tryBufferedImageApproach: Successfully read image: " + width + "x" + height);
            } else {
        ...
        try {
            ...
            if (template.getBackgroundColor() != null && !template.getBackgroundColor().isEmpty()) {
                try {
                    ...
                    System.err.println("[DEBUG] tryCreateNewImageApproach: Invalid color format: " + template.getBackgroundColor());
                }
            }
    ...
    private boolean tryExplicitFormatApproach(Document document, PdfDocument pdf, Certificate template, byte[] imageBytes) {
        try {
    ...
    private void addColoredBackground(Document document, PdfDocument pdf, Certificate template) {
        try {
            ...
            Rectangle rect = new Rectangle(0, 0, pdf.getDefaultPageSize().getWidth(), pdf.getDefaultPageSize().getHeight());
