...
public class CertificateService {
    ...
    public byte[] generateCertificate(Map<String, String> attendee, String eventId) throws IOException {
        ...
        try {
            ...
            // ============ RENDER FRAMES AND DECORATIONS ============
            // Draw frame based on frameStyle
            if (template != null) {
                renderFrame(pdf, template);

                // Draw decorative corners if enabled
                if (template.isDecorativeCorners()) {
                    renderDecorativeCorners(pdf, template);
                }

                // Draw ribbon if enabled
                if (template.isShowRibbon()) {
                    renderRibbon(pdf, template);
                }

                // Draw seal if enabled
                if (template.isShowSeal()) {
                    renderSeal(pdf, template);
                }
            }
    ...
    /**
     * Render frame based on frameStyle from template
     */
    private void renderFrame(PdfDocument pdf, Certificate template) {
        try {
            String frameStyle = template.getFrameStyle();
            if (frameStyle == null || frameStyle.equals("none")) {
                return;
            }
            ...
            float pageHeight = pdf.getDefaultPageSize().getHeight();
            PdfCanvas canvas = new PdfCanvas(pdf.getFirstPage());
            Color borderColor = template.getBorderColor() != null
                    ? parseColor(template.getBorderColor())
                    : ColorConstants.GRAY;
            float borderWidth = template.getBorderWidth() > 0 ? template.getBorderWidth() : 1;
            ...
            switch (frameStyle) {
                ...
                case "classic":
                    float inset = 15;
                    canvas.rectangle(inset, inset, pageWidth - 2 * inset, pageHeight - 2 * inset);
                    canvas.stroke();
                    break;
                ...
                case "double":
                    float outerInset = 15;
                    float innerInset = 22;
                    canvas.rectangle(outerInset, outerInset,
                            pageWidth - 2 * outerInset, pageHeight - 2 * outerInset);
                    canvas.stroke();
                    canvas.rectangle(innerInset, innerInset,
                            pageWidth - 2 * innerInset, pageHeight - 2 * innerInset);
                    canvas.stroke();
                    break;

                case "ornate":
                    float ornateInset = 12;
                    canvas.rectangle(ornateInset, ornateInset,
                            pageWidth - 2 * ornateInset, pageHeight - 2 * ornateInset);
                    canvas.stroke();
                    // Inner decorative line
                    canvas.setLineWidth(borderWidth * 0.5f);
                    canvas.rectangle(ornateInset + 5, ornateInset + 5,
                            pageWidth - 2 * (ornateInset + 5), pageHeight - 2 * (ornateInset + 5));
                    canvas.stroke();
                    break;
                ...
                case "modern":
                    ...
                    canvas.moveTo(modInset, modInset + 30);
                    canvas.lineTo(modInset, pageHeight - modInset - 30);
                    canvas.stroke();
                    canvas.moveTo(pageWidth - modInset, modInset + 30);
                    canvas.lineTo(pageWidth - modInset, pageHeight - modInset - 30);
                    canvas.stroke();
                    // Top and bottom horizontal accents
                    canvas.moveTo(modInset + 30, pageHeight - modInset - 10);
                    canvas.lineTo(pageWidth - modInset - 30, pageHeight - modInset - 10);
                    canvas.stroke();
                    canvas.moveTo(modInset + 30, modInset + 10);
                    canvas.lineTo(pageWidth - modInset - 30, modInset + 10);
                    canvas.stroke();
                    break;
            ...
            canvas.restoreState();
            System.out.println("[DEBUG] renderFrame: Rendered frame style: " + frameStyle);

        } catch (Exception e) {
            System.err.println("[DEBUG] renderFrame: Failed: " + e.getMessage());
        }
    }

    /**
     * Render decorative corners on the certificate
     */
    private void renderDecorativeCorners(PdfDocument pdf, Certificate template) {
        try {
            float pageWidth = pdf.getDefaultPageSize().getWidth();
            float pageHeight = pdf.getDefaultPageSize().getHeight();
            PdfCanvas canvas = new PdfCanvas(pdf.getFirstPage());

            Color cornerColor = template.getHeaderColor() != null
                    ? parseColor(template.getHeaderColor())
            ...
            canvas.saveState();
            canvas.setStrokeColor(cornerColor);
            canvas.setLineWidth(lineWidth);

            // Top-left corner
            canvas.moveTo(offset, pageHeight - offset - cornerSize);
            canvas.lineTo(offset, pageHeight - offset);
            canvas.lineTo(offset + cornerSize, pageHeight - offset);
            canvas.stroke();

            // Top-right corner
            canvas.moveTo(pageWidth - offset - cornerSize, pageHeight - offset);
            canvas.lineTo(pageWidth - offset, pageHeight - offset);
            canvas.lineTo(pageWidth - offset, pageHeight - offset - cornerSize);
            canvas.stroke();

            // Bottom-left corner
            canvas.moveTo(offset, offset + cornerSize);
            canvas.lineTo(offset, offset);
            canvas.lineTo(offset + cornerSize, offset);
            canvas.stroke();

            // Bottom-right corner
            canvas.moveTo(pageWidth - offset - cornerSize, offset);
            canvas.lineTo(pageWidth - offset, offset);
            canvas.lineTo(pageWidth - offset, offset + cornerSize);
            canvas.stroke();

            canvas.restoreState();
            System.out.println("[DEBUG] renderDecorativeCorners: Rendered decorative corners");

        } catch (Exception e) {
            System.err.println("[DEBUG] renderDecorativeCorners: Failed: " + e.getMessage());
        }
    }

    /**
     * Render ribbon decoration on the certificate
     */
    private void renderRibbon(PdfDocument pdf, Certificate template) {
        try {
            float pageWidth = pdf.getDefaultPageSize().getWidth();
            float pageHeight = pdf.getDefaultPageSize().getHeight();
            PdfCanvas canvas = new PdfCanvas(pdf.getFirstPage());

            Color ribbonColor = template.getRibbonColor() != null
                    ? parseColor(template.getRibbonColor())
                    : new DeviceRgb(212, 175, 55); // Gold

            float ribbonX = pageWidth / 2;
            float ribbonY = pageHeight * 0.15f;
            float ribbonRadius = 35;

            canvas.saveState();

            // Draw ribbon circle/badge
            canvas.setFillColor(ribbonColor);
            canvas.circle(ribbonX, ribbonY, ribbonRadius);
            canvas.fill();

            // Inner circle (lighter)
            canvas.setFillColor(ColorConstants.WHITE);
            canvas.circle(ribbonX, ribbonY, ribbonRadius * 0.7f);
            canvas.fill();

            // Star shape in center
            canvas.setFillColor(ribbonColor);
            drawStar(canvas, ribbonX, ribbonY, ribbonRadius * 0.5f, 5);
            canvas.fill();

            // Ribbon tails
            canvas.setFillColor(ribbonColor);
            // Left tail
            canvas.moveTo(ribbonX - 25, ribbonY - ribbonRadius);
            canvas.lineTo(ribbonX - 35, ribbonY - ribbonRadius - 40);
            canvas.lineTo(ribbonX - 20, ribbonY - ribbonRadius - 30);
            canvas.lineTo(ribbonX - 15, ribbonY - ribbonRadius);
            canvas.closePath();
            canvas.fill();

            // Right tail
            canvas.moveTo(ribbonX + 15, ribbonY - ribbonRadius);
            canvas.lineTo(ribbonX + 20, ribbonY - ribbonRadius - 30);
            canvas.lineTo(ribbonX + 35, ribbonY - ribbonRadius - 40);
            canvas.lineTo(ribbonX + 25, ribbonY - ribbonRadius);
            canvas.closePath();
            canvas.fill();

            canvas.restoreState();
            System.out.println("[DEBUG] renderRibbon: Rendered ribbon at position: " + template.getRibbonPosition());

        } catch (Exception e) {
            System.err.println("[DEBUG] renderRibbon: Failed: " + e.getMessage());
        }
    }
     ...
     */
    private void drawStar(PdfCanvas canvas, float cx, float cy, float radius, int points) {
        double angle = Math.PI / points;
        float innerRadius = radius * 0.5f;

        for (int i = 0; i < 2 * points; i++) {
            float r = (i % 2 == 0) ? radius : innerRadius;
            float x = cx + (float) (r * Math.sin(i * angle));
            float y = cy + (float) (r * Math.cos(i * angle));
    ...
    /**
     * Render seal decoration on the certificate
     */
    private void renderSeal(PdfDocument pdf, Certificate template) {
        try {
            float pageWidth = pdf.getDefaultPageSize().getWidth();
            float pageHeight = pdf.getDefaultPageSize().getHeight();
            PdfCanvas canvas = new PdfCanvas(pdf.getFirstPage());

            Color sealColor = template.getSealColor() != null
                    ? parseColor(template.getSealColor())
                    : ColorConstants.LIGHT_GRAY;

            // Position based on sealPosition setting
            float sealX, sealY;
            String position = template.getSealPosition() != null ? template.getSealPosition() : "bottom-right";
            float sealRadius = 40;
            float margin = 50;

            switch (position) {
                case "bottom-left":
                    sealX = margin + sealRadius;
                    sealY = margin + sealRadius;
                    break;
                case "top-right":
                    sealX = pageWidth - margin - sealRadius;
                    sealY = pageHeight - margin - sealRadius;
                    break;
                case "top-left":
                    sealX = margin + sealRadius;
                    sealY = pageHeight - margin - sealRadius;
            ...
            // Outer seal circle
            canvas.setFillColor(sealColor);
            canvas.circle(sealX, sealY, sealRadius);
            canvas.fill();

            // Inner circle
            canvas.setFillColor(ColorConstants.WHITE);
            canvas.circle(sealX, sealY, sealRadius * 0.85f);
            canvas.fill();

            // Middle ring
            canvas.setFillColor(sealColor);
            canvas.circle(sealX, sealY, sealRadius * 0.75f);
            canvas.fill();

            // Innermost white circle
            canvas.setFillColor(ColorConstants.WHITE);
            canvas.circle(sealX, sealY, sealRadius * 0.6f);
            canvas.fill();

            // Dashed inner border
            canvas.setStrokeColor(sealColor);
            canvas.setLineDash(2, 2);
            canvas.setLineWidth(1);
            canvas.circle(sealX, sealY, sealRadius * 0.65f);
            canvas.stroke();

            canvas.restoreState();
            System.out.println("[DEBUG] renderSeal: Rendered seal at position: " + position);

        } catch (Exception e) {
            System.err.println("[DEBUG] renderSeal: Failed: " + e.getMessage());
        }
    }
