...
public class CertificateController {
    ...
    /**
     * Cleanup orphaned certificates (certificates with non-existent events)
     * Available via both POST and GET for better compatibility
     */
    @RequestMapping(value = "/cleanup-orphaned", method = { RequestMethod.POST, RequestMethod.GET })
    public ResponseEntity<?> cleanupOrphanedCertificates() {
        try {
            String result = certificateService.cleanupOrphanedCertificates();
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            System.err.println("Error in cleanup-orphaned endpoint: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Error cleaning up certificates: " + e.getMessage());
        }
    }
} 