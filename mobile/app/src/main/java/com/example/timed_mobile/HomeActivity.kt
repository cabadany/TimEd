...
class HomeActivity : AppCompatActivity() {
    ...
    // private var previousTargetLocation: IntArray? = null // Managed locally or passed if needed for animation
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // The setOnTouchListener for tutorialOverlay previously here for cancellation is removed
        // as the PopupWindow will now handle outside touches.
    ...
    private fun statusOrder(status: String): Int {
        return when (status.lowercase(Locale.ROOT)) {
            "upcoming" -> 0; "ongoing" -> 1; "ended" -> 2; else -> 3
        }
    }
    ...
    override fun onBackPressed() {
        if (drawerLayout.isDrawerOpen(GravityCompat.END)) {
            ...
            currentTutorialPopupWindow?.dismiss() // OnDismissListener will handle cancellation
        } else {
            super.onBackPressed()
        }
    }
    ...
    private fun showTutorialDialog() {
        ...
        layoutQuickTour.setOnClickListener {
            previousTargetLocationForAnimation = null; showQuickTour(); dialog.dismiss()
            val tutorialPrefs = getSharedPreferences(PREFS_TUTORIAL, Context.MODE_PRIVATE)
            tutorialPrefs.edit().putBoolean(KEY_TUTORIAL_COMPLETED, true).apply() // Mark as completed when starting
        }
        btnCancel.setOnClickListener {
            dialog.dismiss()
            val tutorialPrefs = getSharedPreferences(PREFS_TUTORIAL, Context.MODE_PRIVATE)
            tutorialPrefs.edit().putBoolean(KEY_TUTORIAL_COMPLETED, true).apply() // Mark as completed if dialog is cancelled
        }
        dialog.show()
    }
    ...
    private fun showCustomTutorialDialog(message: String, targetView: View, currentStep: Int, totalSteps: Int, onNext: () -> Unit) {
        ...
        popupWindow.setOnDismissListener {
            ...
            // currentTutorialPopupWindow = null; // Handled by handleTutorialCancellation or nextButton logic
        }
        ...
        if (previousTargetLocationForAnimation != null) {
            // Adjust targetView reference if previousTargetLocationForAnimation was based on a different view's center
            val prevTargetCenterX = previousTargetLocationForAnimation!![0] + targetView.width / 2
            val prevTargetCenterY = previousTargetLocationForAnimation!![1] + targetView.height / 2
            ...
            fadeOut.setAnimationListener(object : Animation.AnimationListener {
                ...
                override fun onAnimationEnd(animation: Animation?) {
                    ...
                    if (currentStep == totalSteps) {
                        ...
                        val tutorialPrefs = getSharedPreferences(PREFS_TUTORIAL, Context.MODE_PRIVATE)
                        tutorialPrefs.edit().putBoolean(KEY_TUTORIAL_COMPLETED, true).apply()
                        currentTutorialPopupWindow = null // Clear reference on completion
                        // Toast for completion is now in showAttendanceSectionTour
                    }
                }
        ...
        closeButton.setOnClickListener {
            isProceedingToNextStepOrCompleting = false // Signal cancellation

            val fadeOut = AnimationUtils.loadAnimation(this, R.anim.fade_out)
            fadeOut.setAnimationListener(object : Animation.AnimationListener {
                override fun onAnimationStart(animation: Animation?) {}
                override fun onAnimationEnd(animation: Animation?) {
                    popupWindow.dismiss() // Triggers OnDismissListener which calls handleTutorialCancellation
                }
                override fun onAnimationRepeat(animation: Animation?) {}
            })
            dialogView.startAnimation(fadeOut)
    ...
    private fun showAttendanceSectionTour() {
        val attendanceButton = findViewById<View>(R.id.btntime_in) // Or a more general view for this section
        showCustomTutorialDialog("Ready for an event? Tap 'Time-In' here. You can also 'Time-Out' or send an excuse.", attendanceButton, 4, TOTAL_QUICK_TOUR_STEPS) {
            ...
            // Final completion actions (like hiding overlay, setting prefs) are handled by nextButton's listener in showCustomTutorialDialog
        }
    }
